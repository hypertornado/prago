...
func (resource *Resource[T]) initDefaultResourceActions() {
	newResourceAction(resource, "").priority().Permission(resource.resource.canView).Name(resource.resource.name).IsWide().Template("admin_list").DataSource(
	...
	newResourceFormAction(resource, "new").priority().Permission(resource.resource.canCreate).Name(messages.GetNameFunction("admin_new")).Form(
	...
	newResourceItemAction(resource, "").priority().IsWide().Template("admin_views").Permission(resource.resource.canView).DataSource(
		func(request *Request) interface{} {
	...
	newResourceItemFormAction(resource, "edit").priority().Name(messages.GetNameFunction("admin_edit")).Permission(resource.resource.canUpdate).Form(
		func(form *Form, request *Request) {
			id, err := strconv.Atoi(request.Params().Get("id"))
			must(err)
	...
	newResourceItemFormAction(resource, "delete").priority().Permission(resource.resource.canDelete).Name(messages.GetNameFunction("admin_delete")).Form(
		func(form *Form, request *Request) {
	...
	if resource.resource.activityLog {
		...
		newResourceItemAction(resource, "history").priority().IsWide().Name(messages.GetNameFunction("admin_history")).Permission(resource.resource.canUpdate).Template("admin_history").DataSource(
			func(request *Request) interface{} {
				id, err := strconv.Atoi(request.Params().Get("id"))
				must(err)
