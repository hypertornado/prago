...
func (app *App) initSystemStats() {
	...
	app.Action("_stats").Name(unlocalized("Prago Stats")).IsWide().Permission(sysadminPermission).Template("admin_systemstats").DataSource(
...
func getResourceAccessView(app *App) accessView {
	...
	for _, resource := range app.resources {
		...
		for _, v := range ret.Roles {
			...
			if app.authorize(user, resource.getPermissionUpdate()) {
				s += yeah
			} else {
				s += no
			}
			if app.authorize(user, resource.getPermissionCreate()) {
				s += yeah
			} else {
				s += no
			}
			if app.authorize(user, resource.getPermissionDelete()) {
				s += yeah
			} else {
				s += yeah
			}
			if app.authorize(user, resource.getPermissionExport()) {
				s += yeah
			} else {
				s += no
			}
